...
import {
  fetchDistricteAsync,
  fetchDistricteByIdAsync,
  removeDistricteAsync,
  storeDistricteAsync,
  updateDistricteAsync,
} from "../../../redux/districte/districte.thunk";
import {
  selectDistricteById,
  selectListDistricte,
} from "../../../redux/districte/districte.selector";
import {
...
const Districte = (props) => {
  const { districtesList, getDistricteById, removeDistricte, ...otherProps } =
    props;
      ...
      <div className="page-header">
        ...
        <nav aria-label="breadcrumb">
          <ul className="breadcrumb">
            <button
              className="btn btn-primary text-white display1"
              data-toggle="modal"
              data-target="#exampleModal"
              onClick={() => resetFormAndInitListDistricte()}
            >
              <i className="mdi mdi-plus mdi-18px text-white align-left"></i>{" "}
              Districte
            </button>
          </ul>
        </nav>
                      ...
                      <tr key={districte.id}>
                        ...
                        <td>
                          <div className="row">
                            ...
                            <i
                              className="mdi mdi-pencil mdi-18px text-primary align-left mx-2"
                              data-toggle="modal"
                              data-target="#exampleModal"
                              onClick={() => getDistricteById(districte.id)}
                            ></i>
                            <i
                              className="mdi mdi-delete mdi-18px text-danger align-left mx-2"
                              onClick={() =>
                                deleteDistricte(districte.id, districte.nom)
                              }
                            ></i>
                          </div>
                        </td>
...
const mapDispatchToProps = (dispatch) => ({
  ...
  resetCollectiviteByParentCode: () =>
    dispatch(resetCollectiviteByCodeParent()),
  ...
  getCollectiviteByParentCode: (id) =>
    dispatch(fetchCollectiviteByParentCodeAsync(id)),
});
