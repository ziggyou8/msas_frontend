...
function  StructureForm(props) {
    ...
    const [investissements, setInvestissements]= useState([{libelle: "", montant:""}]);
    const [projets, setProjets]= useState([{libelle: "", opportunite:"", perspective:""}]);
    const [documentInputs, setdocumentInputs]= useState([{name: "", file:null}]);
    const [documents, setDocuments]= useState({});

    const [piliers, setPiliers] = useState([{pilier: "", axe:[{libelle:"", montant_prevu:0, montant_mobilise:0, montant_execute:0}]}]);
   ...
   const onFileChange = event => { 
    ...
    //console.log('🔥🔥', selectedFile)
      ...
      const addPilier = ()=>{
        setPiliers(prevCount => [...prevCount, {pilier: "", axe:[{libelle:"", montant_prevu:0, montant_mobilise:0, montant_execute:0}]}])
     }
    ...
    const addAxe = (index)=>{
       ...
       addedAxeToPilier.axe.push({libelle:"", montant_prevu:0, montant_mobilise:0, montant_execute:0});
       setPiliers([...piliers]);
     }
    const RemoveAxe = (pilierIndex, axeIndex)=>{
      ...
      const addInvestissement = ()=>{
        setInvestissements(prevInvest => [...prevInvest, {libelle: "", montant: ""}])
     }
     ...
     const addProjet = ()=>{
        setProjets(prevProj => [...prevProj, {libelle: "", opportunite: "", perspective: ""}])
     }
    ...
    const totalPillier =(index)=>{
       ...
       return {
           "total_prevu": axes?.reduce((accumulator, current) => accumulator + parseInt(current?.montant_prevu), 0), 
           "total_mobilise": axes?.reduce((accumulator, current) => accumulator + parseInt(current?.montant_mobilise), 0),
           "total_execute":axes?.reduce((accumulator, current) => accumulator + parseInt(current?.montant_execute), 0),
        };
    }
    //hanlers
    const onFileChange = event => { 
...
{                            <div className="form-group col-md-3">
                    ...
                    <div className="input-group ml-1">
                        <div className="input-group-prepend">
                            <span className="input-group-text text-muted" id="basic-addon1">Monnaie</span>
                        </div>
                        <select {...register("monnaie")} onChange={monneyHandler} id="monnaie" className="border" >
                            <option  value="XOF">XOF (CFA)</option>
                            <option value="EURO">EURO (€)</option>
                        </select>
                     </div>
                        {piliers.map((pilier, index)=>(
                            <div key={index} className="row mx-1" style={{ border:'1px solid #C5C3C6'}}>
                            <div className="col-md-2" style={{ margin:'auto'}}>
                                <div className="form-group">
                                    <label htmlFor="pilier">Pilier</label>
                                    <select name="pilier" id="pilier" onChange={pilierLibelleHandler(index)} className="form-control form-control-sm">
                                    <option >Choisir...</option>
                                    {Object.keys(pilierData).map(pilier=><option key={pilier} value={pilier}>{pilier}</option>)}
                                    </select>
                                </div>
                            </div>
                                    ...
                                    <div key={i} className="row no-gutters">
                                        <div className="col-md-2 pl-1">
                                            ...
                                            <label htmlFor="secteur_intervention">Axe</label>
                                                <select name="libelle" id="libelle" onChange={axeInputHandler(index, i)} className="form-control form-control-sm">
                                                <option >Choisir...</option>
                                                {pilier.optionAxies?.map(axe=><option  key={axe[i]} value={axe[1]}>{axe[1]}</option>)}
                                                </select>
                                            </div>
                                        </div>
                                    <div className="form-group col-md-3 col-sm-12 pl-1">
                                        <label htmlFor="montant_prevu">Montant prévu ({monnaie == "EURO" ? '€': "CFA"})</label>
                                        <input type="number" className="form-control form-control-sm" 
                                        name="montant_prevu"
                                        value={axe.montant_prevu || ""}
                                        /* {...register("pilier")} */ onChange={axeInputHandler(index, i)} id="montant_prevu" id="montant_prevu" placeholder={`Montant en ${monnaie == "EURO" ? '€': "CFA"}`}/>
                                    </div>
                                    <div className="form-group col-md-3 col-sm-12 pl-1">
                                        <label htmlFor="montant_mobilise">Montant mobilisé ({monnaie == "EURO" ? '€': "CFA"})</label>
                                        <input type="number" className="form-control form-control-sm" 
                                        name="montant_mobilise"
                                        value={axe.montant_mobilise || ""}
                                        /* {...register("pilier")} */ onChange={axeInputHandler(index, i)} id="montant_mobilise" id="montant_mobilise" placeholder={`Montant en ${monnaie == "EURO" ? '€': "CFA"}`}/>
                                    </div>
                                    <div className="form-group col-md-3 col-sm-12 pl-1">
                                        <label htmlFor="montant_execute">Montant executé ({monnaie == "EURO" ? '€': "CFA"})</label>
                                        <input type="number" className="form-control form-control-sm"
                                        name="montant_execute"
                                        value={axe.montant_execute || ""}
                                        /* {...register("pilier")} */ onChange={axeInputHandler(index, i)} id="montant_execute" id="montant_execute" placeholder={`Montant en ${monnaie == "EURO" ? '€': "CFA"}`}/>
                                    </div>
                                    <div className="form-group col-md-1  mx-auto my-auto " >
                                        <button type="button" className="btn btn-sm" onClick={()=>RemoveAxe(index, i)}><i className="mdi mdi-delete mdi-18px text-danger "></i></button>
                                    </div>
                                </div>
                                ))}
                                
                                <div style={{ marginTop:"-10px" }}>
                                    <button type="button" className="btn btn-sm" onClick={()=>addAxe(index)}><i className="mdi mdi-plus mdi-18px text-primary "><small>Ajouter axe d'intervention</small></i></button>
                                </div>

                                {pilier.axe.length > 1 && <div className="bg-light row w-full no-gutters p-1">
                                        <div className="col-md-3 my-auto">
                                               <small className="text-muted font-weight-bold">{pilier.pilier}</small>
                                        </div>
                                        <div className="col-md-2 mx-4 pl-2 border-left">
                                              <small className="text-muted font-weight-bold">Total prévu</small>
                                              <p style={{ margin: "2px 1px" }}  className="text-muted">{`${totalPillier(index).total_prevu} ${monnaie == "EURO" ? '€': "CFA"}`}</p >
                                        </div>
                                        <div className="col-md-2 mx-4 pl-2 border-left">
                                              <small className="text-muted font-weight-bold">Total mobilisé</small>
                                              <p style={{ margin: "2px 1px" }}  className="text-muted">{`${totalPillier(index).total_mobilise} ${monnaie == "EURO" ? '€': "CFA"}`}</p >
                                        </div>
                                        <div className="col-md-2 mx-4 pl-2 border-left">
                                              <small className="text-muted font-weight-bold">Total exécuté</small>
                                              <p style={{ margin: "2px 1px" }}  className="text-muted">{`${totalPillier(index).total_execute} ${monnaie == "EURO" ? '€': "CFA"}`}</p >
                                        </div>
                                </div>}
                            </div>
                            <div className="form-group col-md-1 " style={{ margin:'auto'}}>
                                <button type="button" className="btn btn-danger btn-sm" onClick={()=>removePilier(index)}><i className="mdi mdi-delete mdi-18px text-white "></i></button>
                            </div>
                        </div>
                        ))}  
                    </div>
                    ...
                    <div className="form-group ">
                        <button type="button" className="btn btn-sm" onClick={()=>addPilier()}><i className="mdi mdi-plus mdi-18px text-primary "><small>Ajouter pilier</small></i></button>
                    </div>
                    
                    {/* investissements */}
                    <div className=" bg-white " style={{ marginTop:'-30px' }}>
                        <div className="row mx-1 pt-3 mb-1 " style={{ border:'1px solid #C5C3C6'}}>
                            ...
                            <div key={index} className="row col-md-6 no-gutters">
                                <div className="form-group col-md-5 col-sm-12">
                                        ...
                                        id="montant_execute" id="montant_execute" placeholder="Investissement"/>
                                </div>
                                <div className="form-group col-md-5 col-sm-12 ml-1">
                                        <label htmlFor="montant_execute">Montant</label>
                                        <input type="montant" className="form-control form-control-sm"
                                        name="montant_execute"
                                        onChange={investissementMontantHandler(index)}
                                        value={investissement.montant}
                                        id="montant_execute" id="montant_execute" placeholder={`Montant en ${monnaie == "EURO" ? '€': "CFA"}`}/>
                                </div>
                                <div className="form-group col-md-1  my-auto" >
                                    <button type="button" className="btn btn-sm" onClick={()=>removeInvestissent(index)}><i className="mdi mdi-delete mdi-18px text-danger "></i></button>
                                </div>
                            </div>
                            )}
                            <div>
                                <button type="button" className="btn btn-sm" onClick={()=>addInvestissement()}><i className="mdi mdi-plus mdi-18px text-primary "><small>Ajouter Investissement</small></i></button>
                            </div>
                      </div>
                    </div>
                    {/* Projets */}
                    <div className=" bg-white mt-1" style={{ marginTop:'-30px' }}>
                        <div className=" mx-1  pt-3 mb-1 " style={{ border:'1px solid #C5C3C6'}}>
                            ...
                            <div key={index}  className="row offset-md-2 no-gutters">
                                ...
                                <div className="form-group col-md-1  my-auto" >
                                    <button type="button" className="btn btn-sm" onClick={()=>removeProjet(index)}><i className="mdi mdi-delete mdi-18px text-danger "></i></button>
                                </div>
                            </div>
                        )}
                            <div>
                                <button type="button" className="btn btn-sm" onClick={()=>addProjet()}><i className="mdi mdi-plus mdi-18px text-primary "><small>Ajouter projet</small></i></button>
                            </div>
                      </div>
                    </div>
